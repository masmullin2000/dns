...
pub async fn run(cfg_str: impl AsRef<str>) -> anyhow::Result<()> {
    ...
    let dot_pool = Arc::new(dot_client::DotConnectionPool::new());
    ...
    if let Err(e) = udp_server(config.clone(), cache.clone(), dot_pool.clone()) {
        error!("UDP server failed: {e}");
        std::process::exit(1);
    }
    if let Err(e) = tcp_server(config.clone(), cache.clone(), dot_pool.clone()).await {
    ...
    loop {
        ...
        dot_pool.cleanup_expired().await;
    }
}
